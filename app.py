...
<div class="hero">
    ...
    <p class="hero-text">
        ...
        риск-анализа и формирования управленческих рекомендаций
...
col_platform = find_column(df_original, ["площадка"])
...
with d1:
    ...
    <p>Выявлены повторяющиеся тематики жалоб и ошибки операторов.</p>
    </div>
    """, unsafe_allow_html=True)
...
with d2:
    ...
    <div class="section-card">
    <h3>Основной риск</h3>
    <p>Накопление обоснованных жалоб снижает качество обслуживания.</p>
    </div>
    """, unsafe_allow_html=True)
...
with d3:
    ...
    <p>Провести адресное обучение и обновить регламенты коммуникации.</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab2:
    ...
    if col_operator:
        ...
        if col_validity:
            df["_Обоснована"] = df[col_validity].apply(
                lambda x: 1 if x == "Обоснована" else 0
            )
        ...
        def risk_level(row):
            if row["Количество_претензий"] >= 10 or row["Доля_обоснованных_%"] >= 70:
                return "Высокий риск"
            elif row["Количество_претензий"] >= 5 or row["Доля_обоснованных_%"] >= 40:
                return "Средний риск"
            return "Низкий риск"
        ...
        if len(high_risk) > 0:
            for name, row in high_risk.head(10).iterrows():
                ...
                Доля обоснованных: {row['Доля_обоснованных_%']}%
                </div>
                """, unsafe_allow_html=True)
        else:
...
with tab3:
    matrix_operator_error = show_heatmap(
        "Heatmap: оператор × ошибка",
        col_operator,
        col_operator_error,
        top_rows=15,
        top_cols=15
    ...
    matrix_topic_reason = show_heatmap(
        "Heatmap: тематика жалобы × причина ошибки",
        col_claim_topic,
        col_error_reason,
        top_rows=15,
        top_cols=15
...
with tab4:
    ...
    if col_claim_text:
        ...
        st.dataframe(
            df[cols_to_show]
            .sort_values("Количество проблемных слов", ascending=False)
            .head(50),
            width="stretch"
        )
...
with tab5:
    ...
    st.markdown("### Генеративный аналитический модуль")

    summary = f"""
    Всего претензий: {total_claims}
    Доля обоснованных: {valid_share:.1f}%

    Топ тематик жалоб:
    {top_claim_topics.to_dict() if 'top_claim_topics' in locals() and top_claim_topics is not None else "Нет данных"}

    Топ ошибок:
    {top_errors.to_dict() if 'top_errors' in locals() and top_errors is not None else "Нет данных"}

    Топ причин ошибок:
    {top_error_reasons.to_dict() if 'top_error_reasons' in locals() and top_error_reasons is not None else "Нет данных"}
    ...
    if st.button("Сформировать экспертный вывод"):
        if not question.strip():
            ...
            prompt = f"""
            Ты — эксперт по управлению контакт-центром медицинской сети.

            На основе аналитической сводки сформируй ответ для руководителя.
            ...
            try:
                with st.spinner("Генеративный аналитический модуль формирует вывод..."):
                ...
                st.error("Ошибка Ollama. Проверьте, что Ollama запущена.")
                st.write(e)
...
{chr(10).join("- " + r for r in recommendations) if 'recommendations' in locals() else "Нет данных"}
"""

st.download_button(
    "Скачать управленческий отчет",
    data=report_text,
